-- Present every question with the same multi-select interaction, without
-- changing the answer key or allowing an extra incorrect selection to score.

update public.pipa_questions
set kind = 'multiple',
    max_selections = 2
where quiz_id = (
...
begin
  ...
  points := case when fully_correct then correct_total * question.points_per_correct else 0 end;
