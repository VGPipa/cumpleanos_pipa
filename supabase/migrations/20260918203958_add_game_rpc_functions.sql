-- Atomic backend operations. Only the Vercel service role can execute them.
...
begin
  ...
  points := correct_selected * question.points_per_correct;
  ...
  insert into public.pipa_game_answer_options (
    session_id,
    question_id,
    option_id,
    is_correct
...
create or replace function public.pipa_submit_rsvp(
  p_session_id uuid,
  p_write_token_hash text,
...
begin
  ...
  insert into public.pipa_rsvps (session_id, quiz_id, status)
  values (game_session.id, game_session.quiz_id, p_status)
...
revoke all on function public.pipa_submit_answer(uuid, text, smallint, text[])
from public, anon, authenticated;
revoke all on function public.pipa_submit_rsvp(uuid, text, public.pipa_rsvp_status)
from public, anon, authenticated;

grant execute on function public.pipa_submit_answer(uuid, text, smallint, text[])
to service_role;
grant execute on function public.pipa_submit_rsvp(uuid, text, public.pipa_rsvp_status)
to service_role;
